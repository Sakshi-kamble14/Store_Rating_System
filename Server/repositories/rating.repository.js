...
// One rating per user per store (unique_user_store). Submitting again updates
...
// All raters + rating values for a set of store IDs (store-owner dashboard).
exports.findRatersForStores = async (storeIds) => {
  if (!storeIds.length) return [];
  const placeholders = storeIds.map(() => '?').join(', ');
  const [rows] = await pool.query(
    `SELECT r.store_id, r.rating, u.id AS user_id, u.name, u.email
     FROM ratings r
...
// Used for admin's "view user details" screen when the user is a Store Owner.
exports.findAverageForOwner = async (ownerId) => {
  const [rows] = await pool.query(
    `SELECT COALESCE(ROUND(AVG(r.rating), 2), NULL) AS avgRating
     FROM ratings r
     JOIN stores s ON s.id = r.store_id
     WHERE s.owner_id = ?`,
    [ownerId]
