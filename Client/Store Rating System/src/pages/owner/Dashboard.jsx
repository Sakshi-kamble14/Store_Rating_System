...
import { formatRating, initials } from '../../utils/formatters.js';
...
const OwnerDashboard = () => {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-xl font-bold text-ink-900">Your Store Overview</h2>
        <p className="mt-1 text-sm text-ink-500">Track your store performance and customer feedback.</p>
      </div>
      ...
      {!error && !loading && stores.length === 0 && (
        <EmptyState icon={Store} title="No stores yet" message="Your account isn't linked to any stores. Contact an administrator." />
      )}

      {!error && !loading &&
        stores.map((store) => (
          <div key={store.storeId} className="card p-6">
            <div className="flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-center">
              <div>
                <h3 className="text-lg font-bold text-ink-900">{store.storeName}</h3>
              ...
              <div className="flex items-center gap-6 rounded-xl bg-ink-50 px-5 py-3">
                ...
                <div className="text-center">
                  <p className="text-xl font-bold text-ink-900">{store.raters.length}</p>
                  <p className="text-xs text-ink-500">Total Ratings</p>
                </div>
              </div>
            </div>
